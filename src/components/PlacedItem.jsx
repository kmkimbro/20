...
export default function PlacedItem({ item, canvasRect, onRemove, onUpdate }) {
  ...
  return (
    ...
    >
      <button type="button" className="placed-image-remove" onClick={handleRemove} aria-label="Remove">
        <RemoveIcon />
      </button>
      {['nw', 'ne', 'sw', 'se'].map((pos) => (
        <div
      ...
      <div className="placed-image-content">
        {item.type === 'uploaded' && item.src && (
          <img src={item.src} alt="Uploaded" />
        )}
        {item.type === 'image' && item.svgContent && (
          <div dangerouslySetInnerHTML={{ __html: item.svgContent }} />
        )}
        {item.type === 'placeholder' && (
          <div className="placeholder-inner">
            <svg viewBox="0 0 260 220" className="placeholder-illustration">
              {PLACEHOLDER_SVG}
            </svg>
