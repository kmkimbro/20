/**
 * DES 49, 50, 53 Combined:
 * - Toolbar above image: Remove, Replace, Retake, 3D View, Crop (DES 53 V2 style)
 * - Broadcast button inside image top-left (DES 53 V2)
 * - Subpages / page viewer in sidebar (DES 49&50)
 * - Screenshot capture enabled (DES 53 V2)
 */
...
export default function PlacedItem_Combined({ item, canvasRect, onRemove, onUpdate, onOpenModal, onNavigateToCad, onStartRetake, previewSelected = false }) {
  ...
  const handleDragStart = useCallback((e) => {
    if (e.target.closest('.resize-handle') || e.target.closest('.placed-image-controls') || e.target.closest('.placed-image-toolbar') || e.target.closest('.placed-image-crop-inner') && cropMode) return;
    setSelected(true);
    e.preventDefault();
  ...
  return (
    ...
    >
      {/* DES 53 V2: toolbar above image boundary */}
      {!cropMode && (
      <div className="placed-image-toolbar" style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 20, pointerEvents: 'auto', transform: 'translateY(calc(-100% - 8px))' }}>
        <div className="placed-image-toolbar-group">
          <button type="button" aria-label="Remove" title="Remove" className="placed-cta-btn placed-cta-btn--danger" onClick={(e) => { e.stopPropagation(); onRemove(item.id); }}>
            <X size={18} />
          </button>
      ...
      {/* Broadcast button - same structure as DES 53 (top-left) */}
      {!cropMode && (
      ...
      {contextMenu && (
        <div
          className="placed-image-context-menu"
          style={{
            position: 'fixed',
            left: contextMenu.x,
            top: contextMenu.y,
        ...
        >
          <button
            type="button"
            className="placed-image-context-menu-item"
